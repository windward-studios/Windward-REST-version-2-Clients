from windwardrestclient.Model import Document
from windwardrestclient.Model import Template, TagTree, VersionInfo, Metrics
import requests
import logging
...
class WindwardClient(object):
    ...
    def licenseKey(self, license_key):
        _HEADERS.update({"X-WINDWARD-LICENSE" : license_key})
        self._licenseKey = license_key

    def getVersion(self):
        try:
            versionUri = self._baseUri + _VERSION_PATH
            response = requests.get(versionUri, headers=_HEADERS)
            try:
    ...
    def createTemplate(self, data, outputType, datasource):
        try:
            templateObject = Template.Template(data=data, outputFormat=outputType, datasources=datasource)
            return templateObject
        except Exception:
            logging.exception("createTemplate() throws ", exc_info=True)
    ...
    def postDocument(self, template):
        try:
            ...
            request = requests.post(documentUri,  headers=_HEADERS, json=template)
            try:
    ...
    def getDocument(self, guid):
        try:
            ...
            response = requests.get(getDocumentUri, headers=_HEADERS)
            try:
                response.raise_for_status()
            except requests.exceptions.HTTPError as httpError:
    ...
    def postMetrics(self, template):
        try:
            ...
            if not isinstance(template, dict):
                template = template.toDict()
            request = requests.post(metricsUri, headers=_HEADERS, json=template)
            try:
                request.raise_for_status()
            except requests.exceptions.HTTPError as httpError:
    ...
    def getMetrics(self, guid):
        try:
            ...
            response = requests.get(getMetricsUri, headers=_HEADERS)
    ...
    def getTagTree(self, guid):
        try:
            ...
            response = requests.get(getTagTreeUri, headers=_HEADERS)
            if response.status_code == requests.codes.ok:
                # print("here")
                response = response.json()
                # print("Response dict: \n", response)
